    ...
    IOptions<FabricSecurityOptions> securityOptions) : ControllerBase
{
    ...
    public IActionResult Pair([FromBody] PairingRequest request)
    {
        if (!string.Equals(request.ExpectedGatewayName, gatewayOptions.Value.Name, StringComparison.Ordinal))
        {
            return Conflict(new PairingResponse(
                "rejected",
        ...
        if (!approved)
        {
            ...
            return Accepted(new PairingResponse(
                ...
                authority.Identity.ClusterId,
                gatewayOptions.Value.Name,
                Message: "Pairing awaits administrator approval or a valid enrollment proof."));
        }

        byte[] csr;
        try
        {
            csr = Convert.FromBase64String(request.CsrBase64);
        }
        catch (FormatException)
        {
            return BadRequest(new { message = "The CSR is not valid base64." });
        }

        byte[] issued;
        try
        {
            issued = authority.IssueCertificate(request.NodeId, request.NodeName, csr);
        }
        ...
        if (enrollmentValidated &&
            request.ChallengeId is { } challengeId &&
            !string.IsNullOrWhiteSpace(securityOptions.Value.EnrollmentToken))
        {
    ...
    {
        if (HttpContext.Connection.RemoteIpAddress is { } remote && IPAddress.IsLoopback(remote))
        {
            return true;
        }
